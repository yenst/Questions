...
let gInterface = (function() {
  let createThreadContainer = function(thread) {
    ...
    let threadContainer = $(
      "<li class='thread' data-id='" +
        ...
        thread.question +
        "</p><div class='row'><ul class='tagscontainer col-7'></ul><form class='col-5' onsubmit='gInterface.addTag(event,this)'><a href='#'onclick='gInterface.showInput(this,event)'><span class='badge badge-primary'><i class='fa fa-plus-square' aria-hidden='true'></i> add tag</span></a><input type='text' class='hide' name='tag'></form></div>" +
        "</div>" +
    ...
    thread.tags.forEach(function(tag) {
      ...
      $(threadContainer)
        .find(".tagscontainer")
        .append(
          "<li class='tagclass badge badge-dark'><a href='#' onclick='gInterface.removeTag(event,this)' data-id='" +
            tag._id +
            "'>" +
            tag.tagname +
            "</a></li>"
  ...
  let createAnswerContainer = function(answer, isTeacher) {
    ...
    answer.answers.forEach(answer => {
      $li
        ...
        .append(createAnswerToAnswerContainer(answer));
    });
    if (isTeacher) {
      $li.append(
  ...
  let updateAnswerToAnswer = function(answer) {
    ...
    $questionWrap.find(".answersToAnswer").append($li);
  };

  let addTag = function(e,t) {
      e.preventDefault();
    let $tag = $(t).find("input[name='tag']");
    ...
    $(t).find('input').addClass('hide');
    console.log($(t).prev());
    $(t).prev().append(
        "<li class='tagclass badge badge-dark f'><a href='#' onclick='gInterface.removeTag(event,this)'>" +
          $tag.val() +
          "</li>"
      );
  };

  let removeTag = function(e,t) {
      e.preventDefault
    let $threadId = $(t)
      .closest("li.thread")
      .attr("data-id");
    ...
    socketModule.sendRemoveTag($threadId, $(t).attr("data-id"));
    console.log($(t));
    $(t).remove();
  };

  let showInput = function(t,e) {
    e.preventDefault();
    console.log($(t).next().removeClass('hide'));
  };
